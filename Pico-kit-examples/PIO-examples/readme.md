...
These examples need a Pico-kit and an RP2350 clone board that is compatible with a Pico2 board but has an
WS2812B LED already mounted on the board. The active code runs on one or more state machines using PIO-0.
Before this code can be loaded, first the [****PIO-assembler****](../../Tools/PIO-assembler-RP2350.f) and [****PIO-disassembler****](../../Tools/PIO-disassembler-v2.f) must be loaded.
