...
: SPI0-ON       ( khz -- )
    ...
    0D bitmask GPIO-OE **bis \ Bit-13 is CS W25Qxxx
    0D bitmask GPIO-OUT **bis \ CS high
    0 spi-master ;
...
v: extra definitions
: &EOL      ( +n -- )       for 0 >data pause next ; \ Fill +n rows with zero
...
: .BITROW   ( a +n -- )      \ Output half of +Nx14 (big) character
    0 >data for
        count >data 1+ pause \ Output every second bit row
    next
    drop  0 >data ;
...
: DEMO      ( -- )
    ...
    begin
        ...
        &cr &"       WORKS"
        10 0 do
            40 ms  s? 0= if leave then
         loop
    s? 0= until  &page ; \ Until a key was pressed

need task
task: OLED

: DEMO  ['] demo  oled start-task  start ;
' demo  to app
v: fresh
