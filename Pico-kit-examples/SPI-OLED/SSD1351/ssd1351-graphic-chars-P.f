...
: .GRAPH    ( a -- )
    4 for
        count  8 for
            onscr? if  dup 1 and >pix  then  2/
        next  drop
    next  drop  x 4 + y xy ;

: GEMIT    ( c -- )
\   onscr? 0= if drop exit then     \ Line full? Yes stop!
    'x 1+  x -  4 < if drop exit then  \ Line full? Yes stop!
    ch 0 -  dup 0< throw  dup 3F >  \ Custom char?
    3 7 slot                        \ Set character space
    if  40 -  2* 2* custom + .graph exit  then
    2* 2* graph +  .graph ;         \ Print graphic pattern
...
: GRAPHICDEMO ( -- )                \ Display graphic token set
    thin  strip  &page
     4  8 xy" Egel project"         \ Startup message
    0C 18 xy" Characters"           \ To line 2
    1A 30 xy" by W.O."              \ To line 4

    800 ms  graphic  &page 0 8 xy
    ch 0 40 bounds do               \ All graphic characters
        x 'x 1+ = if  0 y 8 + xy  then
        i &emit
    loop

    0 28 xy 10 0 do  &" UT"  loop   \ Some patterns, build

    empty  dm 58 0 >cc  dm 60 0 >cc \ two custom chars
           dm 59 1 >cc  dm 61 1 >cc \ out of default chars

    0 40 xy 10 0 do  &" pq"  loop ; \ Show both custom chars
