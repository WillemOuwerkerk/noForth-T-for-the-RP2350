...
D0000000    SIO_BASE, 04=input, 10=output, 30=output enable

: T1      ( -- )
    bus D000,0030 **bic \ Outputs off
    bus D000,0010 **bic \ Outputs active low
    begin
        bus D000,0030 **bix  wait \ Toggle outputs on/off
    key? until
    bus D000,0030 **bic ; \ Outputs off
...
D0000030 constant GPIO-OEN  \ GPIO_OE           output enable register
...
\ Clock stretching, minimum duration of an I2C clock cycle = 228 µsec.
create ISTRETCH     ( -- )
    D0000004 ,  8000 ,  4000 ,
code>
    6822CA70 ,  D101402A ,  D1FA3E01 ,
    CA10C804 ,  FFFF46A7 ,
end-code
...
: BUS!          ( b -- )
    8 for
        ...
        then            wait  2*
        scl gpio-oen **bic  istretch  wait
    next  drop  i2ack@ ;
...
: I2C-ON        ( -- )
    ...
    bus  D000,0030 **bic    \ I2C bus inputs at startup (pulled high)
    bus  D000,0010 **bic ;  \ Init. bus outputs to low

: BUS@          ( -- b )
    0  8 for
        2*  scl gpio-oen **bis  sda gpio-oen **bic  wait
        scl gpio-oen **bic  istretch
        sda gpio-in bit**  0<> 1 and or \ Read bit on correct place now!
        wait
    next
    -1 +to sum
