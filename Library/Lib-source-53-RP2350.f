...
hex v: inside
: ADJUST        ( -- )
    10101000 dup to lib-org  begin  c@+ FF = until  1- to libhere ;
...
\ 1010,1000 value LIB-ORG       \ Start of library
...
v: inside
MAX-CLK
dm 48,000,000   over !  cell+       \ PLL clock
hx 90           over h! 2 +         \ Core voltage 1.0V
hx 3C53         over h! 2 +         \ PLL dividers
1 1 b+b         swap !              \ Peripheral & USB/ADC clock
...
v: inside
MAX-CLK
dm 96,000,000   over !  cell+       \ PLL clock
hx B0           over h! 2 +         \ Core voltage 1V
hx 5052         over h! 2 +         \ PLL dividers
1 2 b+b         swap !              \ Peripheral & USB/ADC clock
4 cfg @ [if]                        \ Is it a duo version?
    ramborder 3C44 + >r             \ MAX-CLK of core1
    MAX-CLK @+      r@  !           \ Copy clock table
    @+              r@ cell+  !
...
v: inside
MAX-CLK
dm 192,000,000  over !  cell+       \ PLL clock
hx 0D0          over h! 2 +         \ Core voltage 1.2V
hx 4041         over h! 2 +         \ PLL dividers 12*64/4
dm 2 4 b+b      swap !              \ Peripheral & USB/ADC clock
4 cfg @ [if]                        \ Is it a duo version?
    ramborder 3C44 + >r             \ MAX-CLK of core1
    MAX-CLK @+      r@  !           \ Copy clock table
    @+              r@ cell+  !
    @               r> cell+ cell+ !
[then]
dm 15,000   0 cfg 2 + h!            \ Set clock to 150MHz
4 cfg @  abs  4 cfg !               \ Restart
v: fresh
config  led
%%
...
v: inside
MAX-CLK
dm 240,000,000  over !  cell+       \ PLL clock
hx 0E0          over h! 2 +         \ Core voltage 1.25V
hx 5041         over h! 2 +         \ PLL dividers
dm 2 5 b+b      swap !              \ Peripheral & USB/ADC clock
4 cfg @ [if]                        \ Is it a duo version?
    ramborder 3C44 + >r             \ MAX-CLK of core1
    MAX-CLK @+      r@  !           \ Copy clock table
    @+              r@ cell+  !
...
v: inside
MAX-CLK
dm 384,000,000  over !  cell+       \ PLL clock
hx 110          over h! 2 +         \ Core voltage 1.4V
hx 8041         over h! 2 +         \ PLL dividers
dm 3 8 b+b      swap !              \ Peripheral & USB/ADC clock
4 cfg @ [if]                        \ Is it a duo version?
    ramborder 3C44 + >r             \ MAX-CLK of core1
    MAX-CLK @+      r@  !           \ Copy clock table
    @+              r@ cell+  !
...
v: inside
MAX-CLK
dm 432,000,000  over !  cell+       \ PLL clock
hx 130          over h! 2 +         \ Core voltage 1.6V
hx 4821         over h! 2 +         \ PLL dividers
dm 4 9 b+b      swap !              \ Peripheral & USB/ADC clock
4 cfg @ [if]                        \ Is it a duo version?
    ramborder 3C44 + >r             \ MAX-CLK of core1
    MAX-CLK @+      r@  !           \ Copy clock table
    @+              r@ cell+  !
...
v: inside
MAX-CLK
dm 480,000,000  over !  cell+       \ PLL clock
hx 150          over h! 2 +         \ Core voltage 1.7V
hx 5021         over h! 2 +         \ PLL dividers
4 0A b+b        swap !              \ Peripheral & USB/ADC clock
4 cfg @ [if]                        \ Is it a duo version?
    ramborder 3C44 + >r             \ MAX-CLK of core1
    MAX-CLK @+      r@  !           \ Copy clock table
    @+              r@ cell+  !
...
chapter .CFG
\ Print the current noForth-T configuration
\ Try to convert an address to name field address, version-2
hex  v: fresh inside also  definitions
noname      ( a1 -- a2 )    \ <SKIP-FF
    3B012503 ,  2EFF781E ,  3D01D102 ,
...
chapter DAS\
here  v: inside \ Complete RP2350 disassembler: DAS MDAS DAS\
need see
need .hex
need -trailing
need [undefined]
...
: CLAIM-PIN ( pin -- )          \ Claim GPIO pin for PIO
    8 *  40028004 + >r          \ Build correct CTRL-pin register address
...
: INPUTS        ( n pin #p -- )     \ Select float, pullup or pulldown for inputs
    ...
    2r> 0 ?do                               \ res pin
        2dup i + pcells 40038004 + >r       \ res pin res
...
: =FREQ     ( u -- )
    >r  0 cfg 2 + h@ dm 10,000 *  r@ /mod \ Sys-clock/Wanted-clock
...
: .SM       ( sm -- )
    ...
    0 cfg 2 + h@ dm 10,000 * 64 rot */ . ." Hz, " \ Calc. & show frequency
...
: SET-FREQ  ( u sm -- )
    >r  >r  0 cfg 2 + h@ dm 10,000 *  r@ /mod   \ Sys-clock/Wanted-clock
    dup FFFF u> ?abort ( Freq. to low )         \ 16-bit overflow?
...
: CORE+     ( -- end start )    \ Boot & auxillary image too
    1010,1000 dup @ +   \ noForth first core
    4 cfg @ if          \ noForth dual core?
        dup @ +         \ Yes, add second core too
    then  xip ;
...
: IMAGE     ( end start -- )    \ Image of noForth
    ...
    repeat
    drop 2drop ." :0000000001FF"
    cr 40 ms ;                  \ To keep USB alive
v: fresh
%%
...
:noname \ START-USB     ( -- )
    10000000  40020000          \ Bit-28 mask & Reset register
...
create RESET1 ( -- )    \ 26 bytes
    40018008 ,          \ Watchdog reset select
    400D8000 ,          \ Watchdog control
    01000000 ,          \ Reset core-1 pattern
code>
...
\ More info on SIO_BASE from address 54 ff.
code BLINKER    ( -- )
(data
    40020000 ,              \ Reset IO-bank to HOP ( RESETS_BASE )
    40028000 r@ 8 * 4 + + , \ Assign GPIOxx to DAY ( GPIOxx_CTRL )
...
data)
    ...
    moon r> # lsls,
    moon  sun 38 #) str,    \ Enable GPIOxx output ( GPIO_OE_SET )
    begin,
...
chapter ADC
hex \ More on ADC chapter 12.4 page 1066 ff
...
chapter PWM-ON
( GPIO -- ) \ PWM is chapter 12.5 from page 1076 ff
...
gpioa 0F and 2/  14 *  400A8000 + constant PWM-CSR  \ PWM control register
...
(* This I2C example is connected to GPIO12 & GPIO13 for I2C0 and
   to GPIO14 & GPIO15 for I2C1. I2C is chapter 12.2 from page 983 ff.
...
sda 2/ 1 and [if]   \ I2C1
40098000 constant 'I2C      \ I2C1_BASE     I2C register pointer
[else]
40090000 constant 'I2C      \ I2C0_BASE     I2C register pointer
[then]
...
I2C is chapter 12.2 from page 983 ff.
...
\ Check for valid SPI start GPIO & select SPI register base address
    ...
    over 08 = or  over dm 12 = or [if]              ( spi-1 )
        ...
        [if] 40088000 [else] 40080000 [then]  constant 'SPI
    [else]
...
v: inside also  definitions
  1000,0000 constant XIP        \ Start of XIP memory
  1010,1000 constant MOV        \ Start of movie
...
SIO_BASE 30 + constant GPIO-OE      \ GPIO output enable
