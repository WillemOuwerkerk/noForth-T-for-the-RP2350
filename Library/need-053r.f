(* NEED vsn 0.53r library mechanism with multiple "files", 1092 bytes
...
\ 1000,0000 constant XIP    \ Start of XIP memory
  1010,1000 value LIB-ORG   \ Default LIB start address
...
: LOAD-CHAPTER ( a b -- )       \ a,b=name, load a source section
    base @ >r  find-chapter     \ Save number base
    ['] refill-chapter          \ Add REFILL for library chapters
    to &refill
    ...
    2r> r> !input  r> base ! ;  \ ib #ib,>in@ source-id restore input source & base
...
v: extra definitions
: COLOR-ON  ( -- )      true to sgr? ;
: COLOR-OFF ( -- )      false to sgr? ;
...
101A5000 to libhere          \ Temporary LIBHERE for restauration
