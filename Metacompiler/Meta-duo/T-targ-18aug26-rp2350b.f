...
\ 03jul2026 Runs with addition of new asembly words: UM* M* / MOD /MOD
\ 08jul2026 First complete running RP2350 solo version, with COLD, COLD2, FREEZE & FREEZE2
...
forth: : ACCEPT     ( a n -- n2 )
    ...
    begin
        begin key dup 80 and while drop repeat
...
inside: : SWALLOW   ( -- )      \ Ignore the rest of the file
    ...
    begin
        ...
        next
            ." ."  cr  exit     \ Yes, show that the file is finished
        then    
        rdrop  ." ,"            \ No, show the skipping of a file
...
align,
label-AMSTERDAM                                         \ a(((
]   !S0  config  5 ms  led      \ Hardware configuration
...
extra: code BOOT  ( -- )
    ...
    pool,
      ...
      ramadr: R0 ,      \ DAY  = Bottom R-stack
      origin ,          \ SUN  = Address of DOES-code
      amsterdam ,       \ MOON = Address of high level BOOT part
    then,
    w  { w hop day sun moon } ldm, \ 5 - Read all startup data
    w  hop ) str,       \ 2 - Set VTOR
    day day ) ldr,      \ 2 - Read R0
    rp day mov,         \ 1 - Set return stack to R0
