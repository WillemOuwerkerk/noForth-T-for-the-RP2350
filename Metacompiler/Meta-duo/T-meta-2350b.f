\ noForth RP2350 -- metacompiler -- 10feb2023 noForth T runs
\ Copyright (C) 2020, Albert Nijhof & Willem Ouwerkerk

(*  noForth T metacompiler ( RP2350 )
...
: PREP-BLOCK    ( -- )      \ Correct for all 68 RP2350 vectors
...
\  Build UF2 binary file for RP2350 bootloader
\
...
vocs? if
    s" noforth tv RP2350 duo 2.bin"
    cr 2dup type  r/o bin open-file \ Add second image file
else
    s" noforth t RP2350 duo 2.bin"
    CR 2dup type  r/o bin open-file \ Add second image file
then
...
: BUILD-UF2   ( a -- )          \ Address range to convert
    ...
    loop  drop  ( space )
\   10041000 to 'target         \ Load second target here
