...
20000000 to IVECS       \ 68 vectors & default code
...
2003FC00 to FLYBUF/
2003FD00 to s0          \ 100 bytes
2003FF00 to r0          \ 200 bytes
2003FF00 to TIB         \ 100 bytes
20040000 to TIB/
20040000 to BORDER      \ Systems end
20040000 to RAMTOP      \ End of user RAM
20081000 to XRAM        \ Tasker RAM
...
: !NOF-NAME ( -- )
    vocs? if
        s" noForth tv RP2350 duo "
    else
        s" noForth t RP2350 duo "
    then  !myname ;
...
: :::NOFORTH::: ( -- )      \ start meta compiling
    ...
    image 10000 ff fill
    cr myname count type  space space here . image . cr
    hot to UHERE
...
: READ-SBOOT ( -- )     \ Open secundairy boot and image file, note size!
    s" boot-nof4.bin" r/o bin open-file \ Add secundairy boot file
