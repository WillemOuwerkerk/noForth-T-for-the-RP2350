\ noForth RP2350 long -- targ = 18124 bytes -- 01jan2023 --
...
20040000        to MSTART  \ Start of binary
...
20000110        to HOT0    \ Hot for first core!
...
20081000        to XRAM    \ Ram for background tasks, etc.
...
label-amsterdam
    day  pc ) ldr,  40078000 ## \ UART1 base address
    begin,
...
\
\                /<<<<<<<<<<<<<<<
\ u FOR offset1 ..code.. NEXT offset2 ..ready..
\           >>>>>>>>>>>>>>>>>>>>>>>>>/
...
hex 40078000           over !  cell+ \ 15 Select UART base address
...
inside: : SWALLOW   ( -- )      \ Ignore the rest of the file
    ...
    begin
        ...
        next
            ." ."  cr  exit     \ Show file finshed
        then    
        rdrop  ." ,"            \ Show file skipping
...
extra: : .LOGO      ( -- )
    cr cr 1B dup spaces ." +"  cr
    cr 7 dup spaces ." +" 1+ dup spaces me count type spaces ." +"
    cr 0F spaces me2 count type
    cr cr spaces ." +" cr ;
...
\    24 25 15000    0 cfg ! \ GPIO pin number for S? & default sys-clk in 10kHz steps
\    hx 40078000    1 cfg ! \ Default UART or 40070000
...
align,
label-AMSTERDAM                                         \ a(((
]   led  !s0  config 5 ms LED   \ Hardware configuration
