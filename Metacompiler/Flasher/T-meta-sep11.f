...
here                      100 allot \ Secundairy boot
...
: PREP-BLOCK    ( -- )
   image 1000 0 fill            \ First erase whole buffer
    20082000 w,                 \ Add highest RAM address
...
: MYNAME2,  s" M33 core with ARMv6-M ISA" dup b, `m, `align ;
...
: `CODE>        dup sys-code ?pair  chere  created `lfa>  w! ;
...
: READ-SBOOT ( -- )     \ Open secundairy boot and image file, note size!
    s" boot-nof3.bin" r/o bin open-file \ Add secundairy boot file
    abort" File failed " >r
...
\   E48BFF57  b!+ ;     \ RP2350 absolute image ID
...
: CALC-BLOCKS ( -- )
    ...
    1 umax to #blocks ;         \ Correct & save number of blocks

: BUILD-UF2   ( a -- )          \ Address range to convert
\   fill-block  block-buffer
\   10 0 do  i dup .  write-block  loop

\   cr binary-buffer 10 write-block drop \ Write secundairy boot to UF2
    MSTART   >hosta? ?abort     \ Write image behind it
    #blocks 0 ?do
        cr i .s write-block     \ Write UF2 block 'i'
    loop  drop ;
...
\   read-sboot   calc-blocks    \ Read secundairy boot
    calc-blocks cr #blocks u.  build-uf2
    cr .s
