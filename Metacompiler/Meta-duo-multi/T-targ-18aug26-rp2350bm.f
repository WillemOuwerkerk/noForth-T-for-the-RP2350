\ noForth RP2040/RP2350 long -- targ = 1xxxx bytes -- 15jul2026 --
...
\ 15jul2026 Started RP2350 duo conversion
...
mstart 00000 +  to IVECS   \ 68 vectors & default code
...
label-cfg
24 25 b+b  15000  h+h  over ! cell+  \ 14 GPIO24 for S?, GPIO25 for led & clockfreq. in 10kHz steps
...
decimal  1             over !  cell+ \ 18 Boot type (0=single, 1=two cores) (158)
...
\               sp
\               ip
extra: code PAUSE      ( -- ) \ 29 machine cycles, is 0.208 microsec. at 150 MHz
...
\ Note that by storing the inverted checksum here the result will be zero
...
extra: : VCORE      ( +n --)
    ...
    r@ or  40100004 !           \ Yes, enable high voltage setting
...
\ Led on
    { hop } pop,                    \ hop 19 # movs,
    w 1 # movs,
    0 4 hop w 0 mcrr,               \ Set GPIO_OUT          AAN1
...
extra: : BOOT1         ( code-addr -- )
    ...
    20040000 >cmd? until    \ Sent interrupt table for core-1, succeed?
    20080000 >cmd? until    \ Sent stack pointer for core-1, succeed?
...
\ note! the address offset 14A may need adjustment
extra: : CONFIG    ( -- )       \ Initialise system clock, etc.
    4 cfg @ 0< 0= if            \ Not already booted?
        ...
        0C 4 ticks  0C 2 ticks      \ Start Watchdog & start Timer0 clocks for MS
        ...
        0 cfg 2 + h@  dup 0A * khz  \ Set default system frequency
        true  4 cfg !               \ Note cores are initialised!
        ramborder 14A + h@ <> if    \ New clock?
            0 cfg 2 + h@  ramborder 14A + h! \ Yes, store clock for second core too!
        then
...
extra: code BOOT  ( -- )
    ...
    pool,
      ivecs ,               \ W   = Interrupt vector tabel
      E000ED08 ,            \ HOP = VTOR
      tp> ,                 \ DAY = Main TCB
      origin ,              \ SUN = DOES address
      amsterdam ,           \ MOON = Address of high level BOOT part
    then,
    w { w hop day sun moon } ldm, \ 3 - Read all startup data
    w  hop ) str,               \ 2 - Set VTOR
    hop day his: r0 #) ldr,     \ 2 - Read R0
    rp hop mov,                 \ 1 - Set return stack to R0
    TP day mov,                 \ 1 - Set task pointer
    does sun mov,               \ 1 - Set call to DOES
...
\ Image-0: 00000 .... 21000 ..... 081000  including secundairy boot
\ Image-1: 81000 .... A1000 ..... 101000
\
...
\ Now: 160 = DP, 14A = Clock freq. in config cell 0 + 2
inside: : IMAGE     ( a -- +n )     FFF +  3F000 and ;
...
( ) ramborder  dup 160 + @ over - IMAGE >r \ Round image-1 to next erase page  roma2 rama2      \r len2r
( ) over r@ wipe-flash                  \ Erase image-1                        roma2 rama2      \r len2r
( ) false cfg 2 + h@  over 14A + h!     \ Copy clock freq. to image-1
( ) r@ over !  r> write-flash  w} ;     \ Write image-1 with rounded length in front


inside: code RESTART    ( a -- )  ( 44 bytes )
    401000B0 ,  \ W     = SCRATCH0
    01FFFFF3 ,  \ HOP   = Reset almost all
    40018008 ,  \ DAY   = WDSEL reset this register
    40100030 ,  \ SUN   = POWMAN: WDSEL register
    400D8000 ,  \ MOON  = WATCHDOG_BASE
code>
