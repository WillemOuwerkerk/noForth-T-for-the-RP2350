(* USB driver used Leon's & Alex Taradov's code & RP2040 datasheet for documentation
    Henny Luijkx made the overview of Alex's code ( 4420 bytes + 648 for tasker words )
...
: ENDPOINTS ( 0 -- 0|+n )   \ Handle used endpoints
    50110058 @ >r
    r@ 04 and if  zlp>      then    \ EP1 active?
    r@ 80 and  or                   \ EP3 active, remember
    r> 50110058 ! ;
...
: USB-HANDLER       ( -- )  \ Handle USB setup & receiving of chars
...
: USB-TX            ( -- )
    begin  4 us usb? until  cell ms \ Connection with driver?
    begin
        4 us  usb? if               \ Still connected?
...
: USB-ON            ( -- )
    ...
    ['] usb-key  to 'key
    ['] usb-emit to 'emit
    ['] usb-handler usb1 start-task     \ Handle setup & char. input handler
    ['] usb-tx      usb2 start-task ;   \ Connection & char. output handler
